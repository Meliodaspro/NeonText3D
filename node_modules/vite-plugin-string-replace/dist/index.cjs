...
var DEFAULT_FILE_NAME = /.*/.source;
var ReplacementMap = class {
  constructor() {
    this.CACHE = {};
  }
  has(k) {
...
var CACHED_REPLACE_OPTIONS = new ReplacementMap();
async function generateReplacementMap(options) {
  for (const { search, replace, fileName, globPattern, globOptions } of options) {
    if (globPattern) {
      const matches = await (0, import_globby.globby)(globPattern, globOptions);
      matches.forEach((m) => {
